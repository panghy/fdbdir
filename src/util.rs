...
use foundationdb::directory::{Directory, DirectoryLayer, DirectoryOutput, DirectoryError};
...
pub async fn dir_for_path(trx: &Transaction, path: &[String]) -> Result<DirectoryOutput, DirectoryError> {
...
pub async fn ls_path(db: &foundationdb::Database, path: Vec<String>) -> Result<()> {
    ...
    db.run(|trx, _| {
        ...
        async move {
            let dl = DirectoryLayer::default();
            if path.is_empty() { println!("/:"); } else { println!("/{}:", path.join("/")); }

            // Directories
            println!("{}", "Directories:".bold());
            let items = dl.list(&trx, &path).await?;
            if items.is_empty() { println!("(none)"); }
            for name in items {
            ...
            if i == 0 { println!("(none)"); }
            if more { println!("{} {}", "…".dimmed(), "use 'scan [limit]' to see more".dimmed()); }
...
pub async fn scan_path(
    ...
    db.run(|trx, _| {
        ...
        println!(
            ...
            prefix.as_ref().map(|p| format!(", prefix {}", format_bytes(p))).unwrap_or_default()
        );
        while let Some(item) = stream.try_next().await? {
            i += 1;
            let key = item.key();
...
pub fn parse_bytes_literal(s: &str) -> Result<Vec<u8>> {
    ...
    while i < bytes.len() {
        ...
        if c == b'\\' && i + 1 < bytes.len() {
            ...
            match n {
                b'x' | b'X' => {
                    if i + 3 >= bytes.len() { return Err(anyhow!("incomplete hex escape")); }
...
fn strinc(mut key: Vec<u8>) -> Vec<u8> {
    for i in (0..key.len()).rev() {
        if key[i] != 0xff { key[i] += 1; key.truncate(i + 1); return key; }
    }
    Vec::new()
}
